...
BEGIN
  ...
  INSERT INTO line_notification_logs (
    ...
    NEW.pharmacist_id,
    'debug',
    'トリガー関数開始: シフトID=' || NEW.id || ', ステータス=' || NEW.status,
    'success',
    NOW()
  );
  ...
  IF pharmacy_record.line_user_id IS NOT NULL AND pharmacy_record.line_notification_enabled THEN
    INSERT INTO line_notification_logs (
      ...
      NEW.pharmacy_id,
      pharmacy_record.line_user_id,
      'shift_confirmed',
      '薬局向けメッセージ（薬剤師: ' || pharmacist_record.name || '）',
      'ready_to_send',
      NOW()
    );
  ELSE
    INSERT INTO line_notification_logs (
      ...
      '薬局向けメッセージ（薬剤師: ' || pharmacist_record.name || '）',
      'failed',
      '薬局のLINE連携未完了または通知無効: line_user_id=' || COALESCE(pharmacy_record.line_user_id::text, 'NULL') || ', notification_enabled=' || COALESCE(pharmacy_record.line_notification_enabled::text, 'NULL'),
      NOW()
    );
  END IF;
