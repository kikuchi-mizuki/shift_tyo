...
-- 1. 既存の関数を削除（戻り値型を変更するため）
DROP FUNCTION IF EXISTS send_queued_notifications();
DROP FUNCTION IF EXISTS send_emergency_notification_immediate(UUID[], DATE, TEXT, TEXT);
DROP FUNCTION IF EXISTS retry_all_failed_notifications();
DROP FUNCTION IF EXISTS get_notification_system_status();
...
BEGIN
  ...
  -- 即座にキューを処理（send_queued_notifications関数を呼び出し）
