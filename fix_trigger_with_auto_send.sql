...
-- 4. コメント追加
COMMENT ON FUNCTION notify_shift_confirmed() IS 'シフト確定時にLINE通知を自動送信する関数（http拡張機能使用）';
COMMENT ON TRIGGER trigger_notify_shift_confirmed ON assigned_shifts IS 'シフトINSERT時に通知を自動送信するトリガー（http拡張機能使用）';
