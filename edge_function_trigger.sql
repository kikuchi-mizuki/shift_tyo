...
BEGIN
  ...
  IF pharmacist_record.line_user_id IS NOT NULL AND pharmacist_record.line_notification_enabled THEN
    ...
    INSERT INTO line_notification_logs (
      ...
      NEW.pharmacist_id,
      'shift_confirmed',
      'Edge Function Response: ' || COALESCE(response::text, 'NULL'),
      CASE WHEN response.status = 200 THEN 'success' ELSE 'failed' END,
      CASE WHEN response.status != 200 THEN 'HTTP ' || response.status::text ELSE NULL END,
      NOW()
    );
  ELSE
    INSERT INTO line_notification_logs (
