...
-- 3. 認証トークンの設定（環境変数として）
-- 注意: 実際の運用では、Supabaseの環境変数設定で行ってください
-- ALTER DATABASE postgres SET app.settings.supabase_anon_key = 'YOUR_ACTUAL_ANON_KEY_HERE';

-- 4. コメント追加
COMMENT ON FUNCTION notify_shift_confirmed() IS 'シフト確定時にLINE通知を自動送信する関数（安定版）';
COMMENT ON TRIGGER trigger_notify_shift_confirmed ON assigned_shifts IS 'シフトINSERT時に通知を自動送信するトリガー（安定版）';
